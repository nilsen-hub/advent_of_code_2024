...
fn hashing_it_out(input: HashMap<usize, usize>, limit: usize) -> usize {
    ...
    while limit > 0 {
        ...
        for (stone, amount) in input_map{
            if stone == 0{
                temp_map.entry(1).and_modify(|count| *count += amount).or_insert(amount);
                continue;
            }
            let len = stone.checked_ilog10().unwrap_or(0) + 1;
            if len&1 == 0{
                let divisor = 10_usize.pow(len / 2);
                temp_map.entry(stone / divisor).and_modify(|count| *count += amount).or_insert(amount);
                temp_map.entry(stone % divisor).and_modify(|count| *count += amount).or_insert(amount);
                continue;
            }
            temp_map.entry(stone * 2024).and_modify(|count| *count += amount).or_insert(amount); 
        }
        input_map = temp_map;
        limit -= 1;
    }
...
fn parse(full_data: Vec<String>) -> HashMap<usize, usize>{
    ...
    for el in split{
        ...
        output.entry(num).and_modify(|count| *count += 1).or_insert(1); 
    }
