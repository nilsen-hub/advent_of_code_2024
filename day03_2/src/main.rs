...
fn memory_cleaner(full_data: Vec<String>) -> Vec<String> {
    let mut output: Vec<String> = Vec::with_capacity(32);
    let mut do_flag = true;
    for mut line in full_data {
        loop {
            if do_flag {
                let end = match line.find("don't()") {
                    Some(val) => {
                        do_flag = false;
                        val
                    }
                ...
                let ok_segment: String = line.drain(..end + 1).collect();
                output.push(ok_segment);
            }

            let start = match line.find("do()") {
                Some(val) => {
                    do_flag = true;
                    val
                }
...
fn babbage(full_data: Vec<String>) -> usize {
    let mut acc: usize = 0;
    let full_data = memory_cleaner(full_data);
